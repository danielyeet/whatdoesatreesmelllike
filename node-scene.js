...
// WHAT IS ON THIS MAP is a CHOSEN FEW of what is in the menu, and the
// owner chooses them. It was briefly every menu entry but Home; they
// then asked for Contact off it, and Photography with it. So the rule
// is not "the map is the menu" — it is that everything here must BE in
...
(function () {
  ...
  const TUBE_SIDES = 8;
  const perRing = TUBE_SIDES + 1;   // one ring of the tube, the seam vertex included
  const REF_PX_PER_UNIT = 94;

  // The swelling where a branch meets the centre: what bridges a
  // hair-thin tube and a sphere twenty-odd times its width, so a branch
  // reads as growing out of the centre rather than as a wire poked into
  // a ball.
  //
  // It is PART OF THE BRANCH — the tube is simply drawn wider for its
  // first fraction of a unit — and not a collar laid over the end of
  // it. A separate piece was two objects however carefully they were
  // matched: it was straight where the branch had already begun to
  // bend, and being transparent over the tube it also came out darker,
  // so what you saw at the middle of the map was seven stubs with seven
  // thin lines starting where they stopped. One object cannot do that.
  //
  ...
  // How far past the sphere the branch finishes taking its own colour
  // back off the centre's. Short on purpose: run long (it was 0.13) and
  // the dark part of the branch reaches well outside the sphere's own
  // outline, where it reads as a black thorn on a pale page rather than
  // as the branch entering the ball.
  const ROOT_FLARE_BLEND = 0.06;
  ...
  /** One branch's geometry: a tube of the given radius that swells out
      ...
      The swelling is made by pushing each ring of the tube outward from
      its own middle, so it follows whatever the curve is doing there
      rather than being a straight piece stuck on the end — which is
      what makes the join invisible. A ring's middle is the average of
      its vertices (the last one repeats the first to close the seam, so
      it is left out of that average), and how far out of the hub that
      middle is is what decides the swelling, measured from the sphere's
      own surface.

      The same pass writes the colour ramp, per vertex. It is a
      multiplier rather than a colour of its own, because the material's
      colour is already kept in step with hover — so this says how much
      of that colour to keep, and the fraction at the hub end is exactly
      the one that turns the tube's own colour into the centre's
      near-black. There is no line to see where the branch enters the
      sphere at all. */
  function branchGeometry(curve, radius, weight, colour) {
    const thin = radius * weight;
    const geometry = new THREE.TubeGeometry(curve, TUBE_SEGMENTS, thin, TUBE_SIDES, false);
    // However heavy the pen, the swelling is the same shape: wide enough
    // to reach the sphere, and never narrower than the tube it belongs
    // to (the hovered tube is already most of the way there).
    const wide = Math.max(ROOT_FLARE_RADIUS * weight, thin);

    const array = geometry.attributes.position.array;
    const keep = [
      COL_CENTRE.r / colour.r,
      COL_CENTRE.g / colour.g,
      COL_CENTRE.b / colour.b,
    ];
    ...
    for (let ring = 0; ring <= TUBE_SEGMENTS; ring++) {
      ...
      for (let j = 0; j < TUBE_SIDES; j++) {
        ...
        cx += array[i]; cy += array[i + 1]; cz += array[i + 2];
      }
      cx /= TUBE_SIDES; cy /= TUBE_SIDES; cz /= TUBE_SIDES;
      const from = Math.sqrt(cx * cx + cy * cy + cz * cz);

      const out = Math.max(0, Math.min(1, (from - CORE_RADIUS) / ROOT_FLARE_LENGTH));
      const grow = 1 + (wide / thin - 1) * Math.pow(1 - out, ROOT_FLARE_CURVE);
      let tone = Math.max(0, Math.min(1, (from - CORE_RADIUS) / ROOT_FLARE_BLEND));
      tone = tone * tone * (3 - 2 * tone);

      for (let j = 0; j < perRing; j++) {
        const i = (ring * perRing + j) * 3;
        if (grow !== 1) {
          array[i] = cx + (array[i] - cx) * grow;
          array[i + 1] = cy + (array[i + 1] - cy) * grow;
          array[i + 2] = cz + (array[i + 2] - cz) * grow;
        }
        for (let c = 0; c < 3; c++) colours[i + c] = keep[c] + (1 - keep[c]) * tone;
      }
      // Past the swelling and past the blend, the rest of the tube is
      // the tube: nothing left to do to it, and the curve only goes
      // further out from here.
      if (out >= 1 && tone >= 1) {
        colours.fill(1, (ring + 1) * perRing * 3);
        break;
      }
    }
  ...
  REAL_NODES.forEach((n, i) => {
    ...
    // One object from the centre to the node: the swelling at the hub
    // end is the tube being drawn wider there, not a second piece.
    function tube(radius, color, opacity) {
      const mesh = new THREE.Mesh(
        branchGeometry(curve, radius, 1, color),
  ...
  function repen(weight) {
    branches.forEach((branch) => {
      [branch.resting, branch.emphasised].forEach((mesh) => {
        mesh.geometry.dispose();
        mesh.geometry = branchGeometry(
          branch.curve, mesh.userData.radius, weight, mesh.userData.colour
        );
        prepareTube(mesh);
      });
    });
  ...
  function animate(now) {
    ...
    //
    // Scaling the rig is what does the work: every branch, speck and
    // waypoint is inside it, so they all converge on the hub together
    // without any of them needing to know this is happening.
