...
Four of those page scripts are elaborate: `chamber.js` (~3,820 lines), `motifs.js`
...
`search.js` (~290), `beaker.js` (~260), `extras.js` (~250), `primer.js` (~230), `landing.js` (~230),
...
- A favourite on the **chamber** page (`favorites`) is an `<a class="gallery-entry">`
  ...
  FRAGRANCE) — and where the opened card reads the start of its writing from. **`data-image`**
...
- **WHAT A FAVOURITE SAYS WHEN ITS CARD IS OPENED** is a
  ...
  or only placeholders (`gallery-waiting`) — opens with the start of its own entry**, fetched
  off the page its `href` points at: the owner's words, never copied here and never edited.
  So a favourite needs no block to say something.
...
| **opening a favourite** | What clicking a card does: it takes the whole width of the grid, the favourites after it travel down a row, and it opens on a measured height into two columns — on the left what is written about that fragrance (the owner's block on this page, or else **the start of its own entry**, read off the page it lives on) and two plain links, **Read the whole entry** (the entry's own `href`) and **Notes** (the site's own notes window in this page's colours); on the right **the print**. One at a time. The card is a `<button>` and not a link, because what it opens carries links; it is opened by a drawn **sign**, a plus in a circle that turns to a minus. It said OPEN ↓ / CLOSE ↑ and GO TO FRAGRANCE in capitals until 2026-09-26 — "less techy, more minimalist and geometric". |
| **the print** (a favourite) | The favourite's picture in an opened card: an upright frame (4 : 5) the picture **fills edge to edge**, nothing behind it, **placed on the bottle** — the bottle found in the picture itself, the picture brought closer where the bottle is small — or, on a clean white or black ground, drawn back until the whole bottle fits, the frame taking that colour — and moved so it stands in the middle (`seat`, `readPicture`, `placeIn` in `chamber.js`, the same reading as the Fragrances view's; a border printed into a picture is trimmed off). Credited under it. `.fav-print`. (Not the chamber's own **plate**, the word and its menu.) |
