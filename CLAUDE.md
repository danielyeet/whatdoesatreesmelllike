...
**A clean run is 310 passed, 0 failed, and takes seven to thirteen minutes.** If you get a
...
- **A z-index worked out from a drawing can outrank the chrome.** A picture on the old
  ...
  layer there is. The axis that stands there now keeps its houses at 4 or under (6 while rested on).
...
| **the say** (the sheet) | The line about a house on the Houses view — "the house that smells like trees" — under the house's label: always, for the house at the **front** of the helix, and for any other while it is pointed at. Every house has one now. |
...
| **the Note Library** | `categories/note-library.html`: every note named in a fragrance on the site — 533 **names as written**, as 330 **records** on 16 **shelves** — each with a brief explanation, stood up as books by `note-library.js`. In the menu above Photography, not on the map. **Almost Human's olfactory landscapes are not in it**: they are impressions, not notes, and were taken out at the owner's word after one round. |
...
| **the front** (houses) | The house on the axis, facing you: the largest, the only one showing its **say** without being pointed at, with a **halo** of specks turning round it. Pressing it opens the house; pressing any other brings that one to the front. |
...
| **resting** (on a house) | The pointer staying on one house for a moment (`HOVER_WAIT_MS`, **200ms** — it was 420 and the owner found it "too long"). Only then do the motifs come and the rest of the page go out of focus (`musing`); sweeping across the houses sets nothing off. |
