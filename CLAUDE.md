...
| `houses/grande-parfums.html` | **Grande Parfums**, the fifth house: seventeen fragrances — fifteen written up alphabetically, two at the foot not smelled yet — over a **drift** of fine specks rising slowly up the window and faint **circles** that swell and gather a perimeter when pointed at, which claims no theme because the owner has not given one | `search.js`, `house.js`, `grande.js` | [the newer houses](docs/features/2026-09-21-the-newer-houses.md) |
...
**A clean run is 259 passed, 0 failed, and takes seven to ten minutes.** If you get a
...
| **Ataraxia** | The fourth house in Scent descriptions: `houses/ataraxia.html`. Five fragrances, none of them named or written yet. The owner says the house's subject is statuary; its **page** is dark gray crossed by **bands** of light, which is a different thing and deliberately so. |
...
| **the circles** (Grande Parfums) | Four to nine faint hairline rings standing about that page, each wandering a little around its own place. Point at one — anywhere on it or inside it — and it **swells**, brightens, and gathers a **perimeter** of specks just outside its rim, which it has only while it is held. Added on 2026-09-23 at the owner's word; they are on top of **the drift**, not instead of it, and the house still claims no theme. `RING_*` and `PERIM_*` in `grande.js`. |
| **the perimeter** (Grande Parfums) | The specks a circle gains while the pointer is on it, turning slowly just outside its rim. It arrives a beat BEHIND the swell and each speck at its own moment, so the circle grows and then gathers — two beats, which reads as waking rather than switching on. |
...
| **the flick** | The pictures going past in the middle window, hard cuts, fast then slowing to a stop. It ends on the picture it keeps rather than cutting to it. **One cut per picture**, over about a second and a half, **in no order and with nothing repeated** — a shuffle without replacement, so each picture is shown once and the one it lands on is held at the start and landed on at the end. How many cuts there are is therefore how many pictures there are, and the slowing is fitted to that rather than the other way round (there is no `FLIP_SLOW` any more). It was a fresh random pick per cut that refused only the same picture twice running until 2026-09-23. **Nothing on the sheet can be pressed while it runs**. The beat before it starts (`FLIP_HOLD_MS`) is not part of that. `FLIP_*` in `contact-sheet.js`. |
...
| **the fragrance reader** | What a fragrance in the **Fragrances** view opens into now: not another page but this one, gone blank, with that fragrance’s picture, writing and an arrow back — and its notes behind the site’s own **View notes** button, which they were not until 2026-09-23: this was the one place a fragrance’s notes were printed flat down the page. The writing is FETCHED from `individual-fragrances/individual-fragrances.html` rather than copied, so there is still one copy of the owner’s words. `fragrance-reader.js`. |
