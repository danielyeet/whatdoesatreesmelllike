...
| `categories/note-library.html` | the **Note Library**: every note named in a fragrance on the site, filed by **accord** and explained — quiet digital **folders** on shelves at night, coloured only on their **tabs**, each with a pixel **glyph** of its own and a name that **decodes** under the hand, read through a catalogue **terminal**, each folder as thick as the note is used, with a **call number** at its foot, opening into a **catalogue card** that links every fragrance using it | `search.js`, `notes-data.js`, `note-library.js` | [the Note Library](docs/features/2026-09-24-the-note-library.md) |
...
`motifs.js` (~1,440), `almost-human.js` (~1,170), `note-library.js` (~1,020),
`pineward.js` (~930), `contact-sheet.js` (~900, down from ~1,570 when the chain replaced
the map, and up again as the hang and then the axis), `adar.js` (~890),
`calculator.js` (~780), `sun.js` (~640), `paper.js` (~580), `moon.js` (~580),
...
| **the data bar** / **the barcode** (library) | A bar across a folder's head filled by how much the note is used — **segmented** like a level meter since the folders became digital files — and a barcode of its own over its call number, drawn off its name — both in grey now. They were the owner's "a little more digitalized", lit in the shelf's colour with a glass body, a cut corner, scanlines and a flicker under the hand; the owner then found it "too annoyingly neony", and the glass, the corner, the scanlines, the flicker and every glow are gone. |
...
| **the glyph** (library) | The five-by-five square of pixels at the head of every folder, drawn off its note's name and mirrored down its middle, as a file's icon is — the folder's own, the same every visit, in grey. `.lib-glyph`, one element, its pixels its box-shadows. |
| **decoding** (library) | What a folder's name does as it comes under the hand: for under half a second it is stray characters settling, left to right, into the name — drawn over the name, which itself never changes. Nothing with reduced motion. |
| **folder** / **tab** (library) | A record as it stands on its shelf: a quiet graphite folder as thick as the number of fragrances that use it, as tall as its own (seeded), its name down it, a **call number** at its foot, and a **tab** standing up off its top edge — the one part in its shelf's colour, at one of three places along the edge in turn. The last on each shelf **leans**. Pressing one **pulls it out** and opens its card. They were **books** (and the owner may still say so) until 2026-09-24; A (quiet books) and C (colour down one edge) were drawn as options and not kept. |
| **call number** | A folder's accord code and its place in it, counted alphabetically — `WOO 007`. Worked out, not written, and unchanged when the books are reordered by use. |
...
| **the catalogue card** | What pressing a folder opens beside the stacks: call number, accord, explanation, other spellings, and **Found in** — the **individual fragrances** first, then **Houses**, each house and then its fragrances, every one linked to its part. **All of it dropdowns**, shut when the card opens (Houses → a house → its fragrances), and what was left open stays open for the next card. It had a bar per house for one round. (Not the theories page's **card**.) |
...
| **the lamp** / **the dust** (library) | The Note Library's room: a soft light following the pointer over the stacks **a beat behind it**, as the cursor's square does (`LAMP_LAG`, the cursor's own 0.16), and specks in the air brighter near it. |
...
| **the morph** | What the arrows do between Chapter 1 and Chapter 2: every speck of the sun, as it stands, is flown into the moon's first frame (and back), on a spiral about the middle, warm tones to cold, on a canvas of its own (`.chapter-morph`), while the writing fades out and back in half way. Both grounds hand their specks over through `capture()`, and the new one is **held** (`hold()`) at the frame the flight is aimed at until it lands, so what comes up is exactly where the specks landed; the old one, glow and all, fades into its own specks over the first fifth. `morph`, `MORPH_*` in `chamber.js`. It used to be a cut, and then for one round still cut at both ends. |
...
| **motifs** | What comes up over the Houses view while a house is **rested on**: that house's own things, taken from its page — Pineward's trees and needles, ADAR's void and dust, Almost Human's figures **glitching into being**, standing a moment and glitching out, and rain, Ataraxia's bands of **particles casting shadows** (a grey haze laid along each — the owner's "gray rectangle" — was taken out), Grande's **bubbles**, rising, wobbling and popping, twice as many as its drift was, Les Abstraits' **droplets concentrating into the house's mark** — its own logo, read off the owner's picture (it was smoke and embers, then abstract compositions, then for part of one night a point, a line and a brush-stroke circle), Tale's doodles, Tombstone's **epitaphs** (its five names cut into the wall letter by letter and worn away) with **roots** creeping in — from the very top edge too — and red flowers opening, whose **petals fall and gather** in a drift along the foot of the window until the house is left, and Qimu & Musicians' **music** — staves written out left to right as complex scores (runs, tuplets, chords, changing times, grand staves), taking turns with **loose music** popping up with no staff. They gather one at a time and **fade** when the house is left, never vanishing, and they stand **behind the houses**, over the whole page. Ataraxia's bands are **fewer and bolder** since the last round. Tombstone writes each of its five names **once**, kept off the houses and their labels and at least 40px from each other; Qimu's are quiet and hardly move. `motifs.js`, named per frame by `data-motif`. Tombstone's were stones in mist for one round, and Qimu's notes, records and a line of sound, then plain staves with a note here and there. |
| **the concentrate** (Les Abstraits) | Les Abstraits' motif since 2026-09-24: 900 pale amber droplets scattered across the page draw in, darkening to ink, onto the house's own four-lobed **mark** (`images/Les-Abstraits/les-abstraits-logo.png`, the owner's), which then sets solid; a drop now and then gathers at its foot and falls. `concentrate()` in `motifs.js`. |
| **loose music** (Qimu) | Passages with no staff — a run under its tuplet, a cadenza of small notes, a cluster, a few chords — popping up on the Houses view while Qimu is rested on, **taking turns** with the staves: nine seconds of staves, seven of loose music. `passage()`, `QIMU_*` in `motifs.js`. |
| **the glitch** (Almost Human's motif) | How a figure comes and goes over the Houses view: cut into bands thrown sideways in steps, with a ghost a few pixels off, settling into a person, standing a moment, and tearing apart again. `GLITCH_IN`, `GLITCH_HOLD`, `GLITCH_OUT` — the owner may want it reversed. (Not the house page's **fault**.) |
