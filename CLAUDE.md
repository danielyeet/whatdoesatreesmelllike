...
Four of those page scripts are elaborate: `chamber.js` (~3,610 lines), `motifs.js`
...
`pineward.js` (~970), `adar.js` (~890),
`calculator.js` (~780), `sun.js` (~680), `fragrance-reader.js` (~640), `moon.js` (~620),
`tale.js` (~590), `paper.js` (~580), `fragrance-line.js` (~560), `abstraits.js` (~510),
...
| **list** / **boxes** / **cards** (fragrances) | The three ways of showing that table, chosen by the three buttons on its right (`.frag-options`, `is-list` / `is-boxes` / `is-cards` on the stage): ruled rows; small squares, several to a row; larger cards with each fragrance's picture, fetched from `individual-fragrances.html` the first time, the hatching and the number where the picture has not arrived. The choice is kept for the next visit in that browser. |
