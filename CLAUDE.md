...
- **A PICTURE IS CREDITED WHERE IT IS USED.** The owner asked for it in as many words,
  ...
  picture is credited under its circle** (*Picture: …*), from its `data-credit` and
...
| **opening a favourite** | What clicking a card does: it takes the whole width of the grid, the favourites after it travel down a row, and it opens on a measured height into two columns — on the left what is written about that fragrance (the owner's block on this page, or else **the start of its own entry**, read off the page it lives on) and two plain links, **Read the whole entry** (the entry's own `href`) and **Notes** (the site's own notes window in this page's colours); on the right **the circle**. One at a time. The card is a `<button>` and not a link, because what it opens carries links; it is opened by a drawn **sign**, a plus in a circle that turns to a minus. It said OPEN ↓ / CLOSE ↑ and GO TO FRAGRANCE in capitals until 2026-09-26 — "less techy, more minimalist and geometric". |
| **the circle** (a favourite) | The favourite's picture in an opened card, **whole** (`contain`) over a blurred copy of itself filling the circle, credited under it. Read once loaded as **a studio shot** (a bottle on white: the circle goes the pale of paper and the picture is multiplied onto it, so the white drops away) or **a scene** (it keeps its blur and is feathered into it). `.fav-disc`, `seat` in `chamber.js`. |
| **the sun** | The ground behind Chapter 1: a sphere far bigger than the window drawn entirely in specks — a Fibonacci surface, three latitude rings and four meridians turning with it, a **registration ring** that does not turn, five **prominences** rising off the limb, and a corona. `sun.js`. Its readability is Ataraxia's, by name and by the same two mechanisms at its own values: a speck over the sheet is drawn at `QUIET` of its strength and its **bloom** is scaled by `hush³`, so the glow is gone long before the writing is. **It answers an opened favourite** (`attend`, since 2026-09-26): a **ring** of its own specks run round the favourite's circle, lit from the sun's side and drawn past the quiet, **ticks** off it, and the sun's own surface round it brightened (`HALO`; not either removed **halo**), turning towards it over a second and away in half of one. |
...
| **list** / **boxes** / **cards** (fragrances) | The three ways of showing that table, chosen by the three buttons on its right (`.frag-options`, `is-list` / `is-boxes` / `is-cards` on the stage): ruled rows; small squares, several to a row; larger cards with each fragrance's picture, fetched from `individual-fragrances.html` the first time, the hatching and the number where the picture has not arrived. **The boxes carry the pictures too** since 2026-09-26, filling the square behind the lettering, and in both every picture is shown **whole** over a **blurred copy of itself** — a studio shot multiplied onto the white, a scene feathered into its blur (`seat` in `fragrance-line.js`). The choice is kept for the next visit in that browser. |
