...
`search-page.js` (~200), `photography.js` (~190), `find-ground.js` (~180), `explorations.js` (~750)
...
- **`prefers-reduced-motion: reduce`** — read by `landing.js`, `paper.js`, `thread.js`,
  ...
  `note-library.js`, `index-page.js`, `explorations.js` (each figure drawn once, still),
...
- **A row on the Explorations & Researches page** also carries `data-kind`: `"Research"` or `"Exploration"`,
  ...
  empty and sorts to the end. It may carry **`data-figure`** too — the figure the field
  gathers into when the row is pointed at: `pyramid`, `resin`, `bottle`, `smoke`, `forest`,
  `rain` or `cloud`. **A row with none still gets one**, by its kind: a Research a molecule
  and an Exploration a terrain, each worked out from the row's own name.
...
| **the field** (RE) | The right two fifths of Explorations & Researches: one drawing in specks, the full height of the window, bracketed at its corners, that the table conducts (`explorations.js`, `.re-field`). With nothing pointed at, **the ring** — a tilted orbit with dust inside, the page's old drawn plate made large. Point at a row and the specks are thrown out from the middle and gather into its **figure**, each on a clock of its own, its hairlines coming up once they have; its **caption** at the foot gives the number, the name and the kind. The specks near the pointer part and darken. Off the table the ring comes back 650ms later; from the keyboard a row's link focused is pointed at; on a phone the written works take turns. (Not the Almost Human page's **clearing** nor any other page's ground.) |
| **figure** (the field) | What the field gathers into for one row, named on the row in `data-figure`: `pyramid` (the primer's, TOP / HEART / BASE), `resin` (a tear with bubbles), `bottle` (with a mist from its cap), `smoke` (COLD and WARM), `forest`, `rain`, `cloud` (a row not written yet). A row naming none gets a **molecule** if it is a Research and a **terrain** (contours and a route) if an Exploration, worked out from its name. |
