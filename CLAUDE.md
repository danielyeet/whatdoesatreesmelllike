...
**A clean run is 158 passed, 0 failed, and takes six to nine minutes.** If you get a
...
| **the burst** | What opening a chapter in the chamber does, in **one continuous movement**: from the press every piece of chrome fades — the menu being **drawn into it**, its rows collapsing towards the middle from the outside in rather than rising back into the word — and the chamber closes on the middle as **two populations** — the ring narrows as a ring along its own orbit, **carrying on at exactly the speed it was already turning at**, while everything still crossing the window keeps the heading it had and is **bent in by the middle** on a curve, each on a clock of its own. They meet; the **home page's own centre** goes out from that point — a dark core with its two halo shells — and a beat later (`WAVE_LEAD`) the black follows it and cuts that chapter's page out. All of that expansion lies **in the ring's own plane**, and it is a **twelve-sided figure** rather than a circle — geometric, and passing **over** the page rather than under it, so its shells bend what is behind them instead of painting a colour over it. Nothing in the wave has a colour. `BURST_*` in `chamber.js`. For the length of it the particle physics is not run at all; the chamber's report says why, and lists six things that were tried and were wrong. |
