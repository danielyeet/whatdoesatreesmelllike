...
The rule is in `instructions.md` (2026-09-25), and in short: **decide and do the code
yourself, and the style too; ask only about the whole and about real ambiguities.** When an
instruction could honestly mean two different things that would come out differently, ask
with the multiple-choice question tool — two to four options each, the recommended one
first — and batch the questions. Never ask about a stylistic choice you can make yourself. Never ask mid-way whether to carry on, to show something first, or to
...
| `categories/researches.html` | **Explorations &amp; Researches**: its name and what it is at the top left, and under them a sortable, searchable **table** across the left three fifths, each row saying whether it is a research or an exploration — and on the right **the field**, a cloud of specks the table conducts, turning in three dimensions: a ring, until a row is pointed at and the specks are thrown out and gather into that row's abstract **form** | `search.js`, `index-page.js`, `explorations.js` | [the field](docs/features/2026-09-26-the-explorations-field.md), [index and views](docs/features/2026-09-17-the-index-pages-and-views.md) |
| `categories/note-library.html` | the **Note Library**: every note named in a fragrance on the site, filed by **accord** and explained — in **black and white**, **an archive**: each accord a **box** with a **lid** saying what is in it, each note a **file** standing in it, its **tab** carrying its **file number** along an edge in its accord's colour and a small square at its foot for every fragrance using it, read through a catalogue **terminal** with a **×** to clear it, and pulled to open its **case file** — hairlines and specks, a stamp, **the mark** clipped in as its exhibit — that links every fragrance using it | `search.js`, `notes-data.js`, `note-library.js` | [the Note Library](docs/features/2026-09-24-the-note-library.md) |
...
| `houses/grande-parfums.html` | **Grande Parfums**, the fifth house: seventeen fragrances — fifteen written up alphabetically, two at the foot not smelled yet — over a **drift** of specks rising up the window and **bursting** into finer ones — as strong as its hover on the Houses view since 2026-09-25 — on champagne paper, which claims no theme because the owner has not given one | `search.js`, `house.js`, `grande.js` | [the newer houses](docs/features/2026-09-21-the-newer-houses.md) |
| `houses/les-abstraits.html` | **Les Abstraits**, the sixth house: four fragrances, written, all four with notes off the house's own page — with a **drawing** of Des Cendres' road standing in its writing, and a **last word** at the foot whose link opens a new window; on iris paper, with an **armoire** of specks in the left margin, clothes and iris in it, and a slow **drip** down the right margin the whole length of the page into a **beaker** at its foot, drawn as a diagram, which overflows | `search.js`, `house.js`, `beaker.js`, `abstraits.js`, `notes-data.js`, `notes.js` | [the newer houses](docs/features/2026-09-21-the-newer-houses.md) |
...
| `houses/tombstone.html` | **Tombstone**, the eighth house: five fragrances in alphabetical order, two pictures each, written — all but the rest of 3 Feet 5 — with a **definition** shown on hover in Evergrow; on ash-grey paper, with **stones** gathering and going in the margins over their **reflections** | `search.js`, `house.js`, `tombstone.js`, `notes-data.js`, `notes.js` | [the newer houses](docs/features/2026-09-21-the-newer-houses.md) |
| `houses/qimu-and-musicians.html` | **Qimu & Musicians**, the ninth house: four fragrances, one to a player in a band — Guitarist and Vocal written, Drummer saying "Description coming soon." at the owner's word, Bassist waiting; on semi-light blue paper, with a quiet **score** in the margins, played as you read | `search.js`, `house.js`, `qimu.js`, `notes-data.js`, `notes.js` | [the newer houses](docs/features/2026-09-21-the-newer-houses.md) |
...
Four of those page scripts are elaborate: `chamber.js` (~4,060 lines), `motifs.js`
(~2,150), `structure.js` (~1,580) and `node-scene.js` (~1,530). The rest are smaller:
`almost-human.js` (~1,170), `contact-sheet.js` (~1,000), `note-library.js` (~990),
`pineward.js` (~970), `adar.js` (~890), `sun.js` (~800),
`calculator.js` (~780), `fragrance-reader.js` (~640), `moon.js` (~620), `fragrance-line.js` (~600),
`tale.js` (~590), `paper.js` (~580), `abstraits.js` (~510),
`notes.js` (~490), `qimu.js` (~460), `essay.js` (~440), `ataraxia.js` (~410),
...
`search-page.js` (~200), `photography.js` (~190), `find-ground.js` (~180), `explorations.js` (~500)
...
`.library-page`, `.ataraxia-page` and — on paper of their own since 2026-09-25 —
`body.grande-page`, `body.abstraits-page`, `body.tombstone-page` and `body.qimu-page`
...
**A clean run is 393 passed, 0 failed, and takes seven to twenty minutes.** If you get a
...
- **`prefers-reduced-motion: reduce`** — read by `landing.js`, `paper.js`, `thread.js`,
  ...
  `note-library.js`, `index-page.js`, `explorations.js` (each form drawn once, still),
  `fragrance-reader.js`, `fragrance-line.js` (the mark
  still, the items simply there and a change of layout at once),
  `views.js` (no crossing, no swipe, no fade — the views simply change over) and
  `style.css` (which also turns off every house's way in), each degrading to a still
  version. `nav.js` (the cursor) and `extras.js` do *not* check it beyond that; if you
  add motion there, add the guard too.
...
| Each house's motifs, and every house's way in (and the chain, replaced by the hang) | `motifs.js`, `beaker.js` | [report](docs/features/2026-09-23-the-chain-and-its-motifs.md) |
...
| Ataraxia, Grande Parfums, Les Abstraits, Tombstone and Qimu & Musicians, and their grounds | `house.js`, `ataraxia.js`, `grande.js`, `abstraits.js`, `tombstone.js`, `qimu.js` | [report](docs/features/2026-09-21-the-newer-houses.md) |
...
- **New piece of work**: duplicate a template in `works/` — `example-gallery-work.html`
  ...
  Fragrances view) takes another `<tr>` — which on Scent descriptions is also, on its own,
  another item of the whole-page table, in all three of its layouts (`fragrance-line.js`
  reads the old table). Each page says which in the comment at the top
  of it.
...
- A favourite on the **chamber** page (`favorites`) is an `<a class="gallery-entry">`
  ...
  fragrance lives on the site, and is what **Read the whole entry** follows (it was GO TO
  FRAGRANCE). **`data-image`**
  is its picture, the same file its own page shows, with **`data-credit`** (whose picture, as
  its own page credits it) and **`data-credit-href`** (the link, if there is one); every
  favourite carries all three since 2026-09-26. The chapters are the
...
- **What a chapter is** is written in its own
  ...
  note about the favourites not being ranked stands. A **`<p class="gallery-after">`** in the
  block is taken out of the description and stood **under the chapter's last card** instead —
  the end of the list — which is where Chapter 1's three from ADAR's Aegis collection that
  "could have all made it here too" stand. **`data-ground`** on the block is the
...
- **WHAT A FAVOURITE SAYS WHEN ITS CARD IS OPENED** is a
  ...
  means renaming it in both places in the same turn. It is **the owner's own paragraphs**,
  as plain `<p>`s, exactly as they wrote them — all ten carry one since 2026-09-26 — and a
  paragraph marked `gallery-word` is set apart as commentary, if one ever is. **A favourite
  with no block opens with its picture and its two links and nothing else**: the start of its
  own entry, read off the page it lives on, was shown there until the owner wrote all ten,
  and that went with the placeholders (`gallery-waiting`).
- **A NEW HOUSE** is `houses/<house>.html` on `body.human-page`, loading `search.js` and
  **`house.js`** — the shared house shape, which gives it the parts opening on a measured
  ...
  one nearly invisible); a house with no theme yet simply has neither. Its head is its
  name and a subtitle **in title case** (`<h1>Name<br><em>A House That …</em></h1>`) and
  **nothing under it** — no standfirst; there is a test for both. Then: a frame on the Houses view (with a `data-motif`, and motifs for it in `motifs.js`), a line in `PAGES` in `search-page.js`,
...
- **A NOTE IN THE NOTE LIBRARY** is one `<article class="lib-record">` in
  `categories/note-library.html`, in the `<section class="lib-shelf">` of its family — its
  **accord**, as the page calls it (the markup still says shelf) — with
  its explanation in a `<p class="lib-say">`; every other spelling the site uses for the
  same note goes in its `data-aka`, separated by `|`. It is filed as a **file** in that
  accord's **box**, its file number worked out from where it stands. **Which fragrances use it is worked
...
- **A row on the Explorations & Researches page** also carries `data-kind`: `"Research"` or `"Exploration"`,
  ...
  empty and sorts to the end. **Nothing else is needed for the field**: a row's form comes
  from its number, and an Untitled row with no kind is the plain cloud.
...
| **the long move** | The 2.4-second scroll between slide 2 and the map, either way (the others take 1.1s). It eases on a **sine** since 2026-09-26 (`easeLong` in `landing.js`; it was a cube, which barely moved for its first third of a second), and the page is moved at the **head of every frame** — `landing.js` keeps one loop, `tick`, started before the drawings start theirs — so the curtain, the grid, the map and the thread all read where the page is rather than where it was a frame before. See the slides' report. |
...
| **link node** / **real node** | A clickable endpoint from `REAL_NODES`. A branch *stops* at one; nothing continues past it. There are **eight** since the Note Library joined (2026-09-24), and their positions are a **Fibonacci sphere** rather than points placed by hand: every branch the same length and the closest pair 66.3° apart, the whole turned 80° round the upright axis so that no two labels touch at rest (measured, not guessed — see the node map's report). Adding a ninth means recomputing the whole list, and measuring the turn again. |
...
| **chapter** | One grouping in Favorites — whatever an entry's `data-chapter` says, **plus any chapter written up in a `.gallery-chapter` block with nothing filed under it**. The chapters, their names and their order all come from the page. There are two: Chapter 1, which has ten favourites (since 2026-09-25; its two placeholders were taken off) and the sun behind it, and Chapter 2, which is *To be determined...* and empty. Chapter 3 was removed at the owner's word. |
...
| **the kindle** | What answers the hand on Ataraxia: the specks within reach of the pointer burn brighter, eased in and out — and each one the hand has passed over **lingers**, lit for half a second after it has gone and going out over the next second and a half (`LINGER_*`), at the owner's "a delay of the particles turning off after you hover them". |
...
| **the drift** (Grande Parfums) | That page's ground: specks rising up the window, with a few larger **motes** among them; they **lean** towards the pointer rather than being pulled to it, and are drawn quietly over the writing. They **burst** into a spray of finer specks — the owner's "particles that rise up and pop more or less into a bunch of other smaller particles", and never drawn bubbles. It was the quietest thing on the site, half of them bursting on the way up; since the last round of 2026-09-25 — "Intensify the particles in grande parfums particle page (make it like the hover in SD)" — it is drawn as the hover is: up to 640 at once, heavier, each born somewhere in the lower four fifths of the window, rising for four to eight seconds and **every one** bursting, then born again elsewhere (`LIVE`, `born()`). On champagne paper. `grande.js`, `BURST_*`. |
| **it says nothing about the house** | Why the drift is what it is. Every other drawing here is its house said as a behaviour — a wood, a void, a crowd, bands of light. The owner still has not said what Grande Parfums is (*"Idk the theme to be honest"*), so its ground is a ground rather than a statement — the bursting is what they asked for, not a theme. **When they say, `grande.js` is the file to replace**, not to extend. |
...
| **Les Abstraits** | The sixth house: `houses/les-abstraits.html`, *Eugen’s ideas and Antoine Lie’s execution*. Four fragrances — Belle Âme, Des Cendres, La Douleur Exquise, Philosopher's Walk — **written** since 2026-09-23. One sentence in the introduction stopped half way — *The vibe I get from Les Abstraits is that it is stuff* — and stood so because the owner left it so, until they asked for it out (2026-09-25). |
...
| **the wells** / **the bend** (ADAR's motif) | What ADAR's soundings are on the Houses view since the night of 2026-09-25 — "a black hole effect on anything they touch": a hole with a disk of specks spiralling into it, and **waves of dots** ringing out much further than the soundings did. Together they make a field (`bendPoint`, handed out by `HouseMotifs.bend()`) that pulls, turns, shrinks and swallows whatever is drawn through it — and `contact-sheet.js` draws the whole view through it: the specks, the axis, the numbers and the houses (never the one rested on) — while the page's own squares are drawn bending into each hole (`spacetime`). Up to three at once, **a third rarer** since the last round of 2026-09-25 (`rate` 0.3), and **never next to each other**: a well is born only `WELL_APART` (0.55 of the window's shorter side) from every other, or not at that moment. (Not ADAR's own page's **void**.) |
...
| **the summary plate** | **Off the page** since 2026-09-25 ("Remove the picture from the Note dissemination framework, and all other theories" — the other two theories' empty plates went too). `images/Theories/note-dissemination-summary.png`: the whole of the theory on one sheet, which stood at the top of the piece and on its **card** on the theories page. Generated rather than drawn, from the same script that made the piece's diagrams; the file is kept, and nothing points at it. The piece's diagrams are not pictures and stay. |
...
| **Explorations & Researches** | What the **Researches** category is called now (`categories/researches.html`, which keeps its filename). It was **Works** for a round, which the owner then made specific. The owner calls it **RE**. A third column says whether each piece is a **research** or an **exploration**. Since 2026-09-26 it is laid out for itself: the name, the owner's paragraph under it (the **Information** heading it stood under is gone), the table across the left three fifths, and **the field** on the right; the line and the plates that stood there are gone. |
| **the field** (RE) | The right two fifths of Explorations & Researches: one drawing in specks, the full height of the window, bracketed at its corners, that the table conducts (`explorations.js`, `.re-field`). Everything it draws is **a cloud**: a haze of specks gathered round a geometric **form** in three dimensions, turning slowly about an axis leaning towards you, the nearer specks larger and darker, a faint **web** of hairlines between a few of them. With nothing pointed at, **the ring**. Point at a row and the specks are thrown out from the middle and gather into its form, each on a clock of its own; its **caption** at the foot gives the number, the name and the kind. The specks near the pointer part and darken. Off the table the ring comes back 650ms later; from the keyboard a row's link focused is pointed at; on a phone the written works take turns. |
| **form** (the field) | What the field gathers into for one row, by the row's **number**, round a list of ten: sphere, knot, torus (tipped), helix, disc (a spiral), lattice (a cube's edges), gyre (three rings crossed), saddle, shells (three, nested), hourglass. An Untitled row with no kind is **the cloud**, the one form with no shape in it; nothing pointed at is **the ring**. Abstract on purpose: the owner's "a general abstract geometric particulate thing ... The closest thing to waht i like is the cloud". |
| **figure** (the field) | **Removed** (2026-09-26, the same day it came): a drawing for each work — the primer's pyramid, a tear of resin, a bottle, two smokes (COLD / WARM), firs, rain, and a molecule or a terrain for rows to come — named on the row in `data-figure`. The owner: "REmove the research specific stuff". No `data-figure`, no figure names and no words in the field anywhere. |
| **the primer** | *My Personal Introduction to Perfume*, `works/my-personal-introduction-to-perfume.html` — Explorations **000**, the first row on that page. An essay page in **gold**, with the diagrams the owner asked for: oil + alcohol (the perfume holding exactly the two), the concentrations as overlapping bars from the owner's own table, the accords table, and two pyramids that are one true triangle cut in three. Its footnotes and its sources are sections **13 Footnotes** and **14 Citations**, so both stand on the rule. |
...
| **the say** (the sheet) | The line about a house on the Houses view — *The House That Smells Like Trees* — under the house's label, **only while it is pointed at**, the front house's too (it showed on the front one always for one round). **In title case**, as a book's title is set, at the owner's word — and so is every house's subtitle on its own page; Ataraxia's is its subtitle. There is a test. |
| **index** | The way `categories/researches.html` was laid out until 2026-09-26 (see **the field**), and the way the old Fragrances index kept on Scent descriptions still is: readings across the top, plates on the right, and a sortable, searchable table in the bottom left corner. `index-page.js`. The contact sheet's **Fragrances** view is built from the same markup and script but laid out again for itself — one centred column, the table given the room — under `body.view-fragrances`. The copyright line that used to sit under the board is gone from both. |
...
| **the Note Library** | `categories/note-library.html`: every note named in a fragrance on the site — 332 **records** in 16 **accords** — each with a brief explanation, filed as **files in boxes** by `note-library.js` (it stood them up as books until 2026-09-26). In the menu above Photography, and the eighth node on the map. **Almost Human's olfactory landscapes are not in it**: they are impressions, not notes, and were taken out at the owner's word after one round. |
...
| **record** (library) | One note in the Note Library: an `<article class="lib-record">` with its name, the other spellings folded into it (`data-aka`), and what it is (`.lib-say`). Filed as a **file**. |
| **accord** (library) / **shelf** | One family of notes in the Note Library — Citrus, Woods, Impressions and so on — with a three-letter **code** (`CIT`, `WOO`, `IMP`), a colour of its own (the only colour on the page since it went black and white, 2026-09-26: on its tab under the search, its code, the strip down its **box**'s lid, the edge of each of its files' tabs, and its card's specks), and its files standing in **a box** of its own. The page calls it an **accord** since the owner asked ("Replace the word shelves with accords"); the markup and the code still say shelf (`.lib-shelf`, `data-shelf`), which nobody reads. (Not the primer's **accords table**, which is perfumery's own sense of the word.) |
| **the glyph** (library) | **Removed** (2026-09-25): the five-by-five square of pixels at the head of every folder, drawn off its note's name — the owner's "logos on the books ... unneccessary". No `.lib-glyph` anywhere. |
| **decoding** (library) | **Removed** (2026-09-25): a folder's name settling out of stray characters as it came under the hand. Part of what made the page "3-bit". |
| **the box** / **the lid** / **the hand-hole** (library) | What an accord is since 2026-09-26, last — the owner's "like files in boxes rather than a library ... movies and spy stuff": a hairline box, open at the top, its **lid** (`.lib-plate`) across its head a few pixels proud of it either side with a rivet at each end — **BOX 04** (numbered in the order the accords stand on the page, `.lib-box-no`), the accord's code in its colour, its name and line, *39 FILES* — and a strip of its colour down the lid's left end; a rounded **hand-hole** cut in its front at the foot (`.lib-records::after`). |
| **file** / **tab** / **file number** / **the marks** (library) | A record as it stands in its box: a **file** folder in the page's greys, its name typed on its face; its **tab** (`.lib-call`) on its top in one of three places in turn (`--tab`), as a drawer's are staggered, carrying its **file number** — `WOO-007`, the old **call number** with a dash: the accord's code and its place in it, counted alphabetically, worked out, and unchanged when the files are ordered by use — with **its accord's colour along the tab's top edge and nowhere else on the file**; and at its foot **the marks** (`.lib-marks`), a small square for every fragrance using the note, up to eighteen (`MARKS_MOST`) and `+n` past that — so a note used often is a thicker file. Pressing one **pulls** it up out of its box (`is-out`) and opens its **case file**. (Not the old **folders**, below.) |
| **the ×** (library) | The small square with a cross in it at the right end of the Note Library's search bar (`.lib-clear`), after the count: faint and not pressable while the bar is empty, and pressing it empties the bar and brings every box back. The owner's "a little X to reset the search on the right side of the bar". The browser's own clearing mark is taken off that field. |
| **book** / **spine** / **cloth** / **the lean** / **the slip** (library) | **Removed** (2026-09-26, last) for **files in boxes**. For four rounds a record stood on its shelf as a **book** as thick as its uses, its name down its **spine** on a canvas of its own (`drawSpine`, `.lib-spine`) — in its accord's **cloth** with gilt (`CLOTH`, `GILT`), first drawn in specks ("granular"), then in flat shapes, and last as a hairline **skeleton** with its accord's colour in one band across its head — the last on each shelf **leaning**, and a **slip** with its line coming up beside it under the hand. Before the books it was **folders** — graphite files coloured only on a tab, then "digital" ones the owner found "3-bit" (2026-09-24). Nothing of any of it is in the code. |
| **call number** / **the label** (library) | What a **file number** was called while the records were books: `WOO 007`, printed in **the label**, a box at the foot of the spine. **The label is removed** with the books; the number is the file number now. |
| **the boards** / **the case** / **the label holder** (library) | **All removed** — the boards with the books (2026-09-26, last), the case and the rail before them. What an accord's books stood on: **a board** under every row and nothing round it (`canvas.lib-boards`, `drawBoards`, `BOARD_*`) — flat walnut with a cream card of call numbers (2026-09-25, last), then drawn in hairlines (2026-09-26). Before that **the case**, a bookcase of walnut drawn in specks round every accord with a brass **label holder** round each card (the night of 2026-09-25; the owner's "remove the bezel of the bookshelves"); before the case **a rail**, one lit line under each row. None of it is in the code: no `drawBoards`, no `drawCase`, no `.lib-case`. An accord is **a box** now. |
| **the terminal** (library) | The `query>` field over the Note Library, with **the ×** at its right: files that answer light up, the rest go dim, empty boxes fold away. It reads names and other spellings, by **direct words**. |
| **the catalogue card** / **the case file** | What pressing a file opens beside the archive — since 2026-09-26, last, **a case file**: *CASE FILE* and the file number across its head, a **stamp** across it at a slant (*DECLASSIFIED*), **the mark** clipped in as its **exhibit** with a paperclip and a caption (*Exhibit A · 28 appearances*), then **SUBJECT** (the name, its accord), **SUMMARY** (what it is), **ALIASES** (the other spellings; it said *Also catalogued as*) and **KNOWN APPEARANCES** (it said *Found in*) — the **individual fragrances** first, then **Houses**, each house and then its fragrances, every one linked to its part. **All of it dropdowns**, shut when the card opens (Houses → a house → its fragrances), and what was left open stays open for the next card. Drawn in hairlines with a registration tick at each corner and **the mark** at its head since 2026-09-25 — it was a glowing glass panel, which the owner found "too futuristic". It had a bar per house for one round. (Not the theories page's **card**.) |
| **the returns cart** | The box (`RET`, in grey) the Note Library puts at its end for any note `notes-data.js` names that no record carries. Empty, and a test keeps it so. |
| **the lamp** / **the dust** (library) | The Note Library's room: a soft white light following the pointer over the boxes **a beat behind it**, as the cursor's square does (`LAMP_LAG`, the cursor's own 0.16), and specks in the air brighter near it. The scan that passed down the window now and then was taken out with the glass (2026-09-25). |
| **the mark** (library card) | The particle figure at the head of the catalogue card: a ring of specks, one for every fragrance on the site that uses the note, joined one to the next by hairlines, turning slowly in a cloud of its accord's dust, with a registration cross at its centre and a ticked rule run in to it from either side. `.lib-card-mark`, `markFor`, `drawMark`. (Not Almost Human's **mark**, nor the index page's.) |
...
| **the mark** | The one plate on an index page that is drawn rather than photographed: a slow ring of specks with lines between the near ones — the chamber's orbit printed small, on white. Only on the old Fragrances index kept on Scent descriptions now; Explorations & Researches' plates went on 2026-09-26, and its **field** turns in the same ring made large. |
...
| **the arrows** (a chapter) | The two buttons either side of a chapter's name, stepping one chapter along and wrapping round. Nothing of the **burst** is replayed: the sheet fades, the page is rewritten under it, and its entrance is run again. They are taken off the page when there is only one chapter to be on. Between two chapters that each have a drawing, **the morph** runs; a press while it (or any step) is running is **remembered** and taken when it lands, rather than dropped. `stepChapter` in `chamber.js`. |
| **the morph** | What the arrows do between Chapter 1 and Chapter 2: the sun turns into the moon **where it stands** (and back), warm tones to cold, on a canvas of its own (`.chapter-morph`), while the writing fades out and back in half way. The two stand on **one sphere** (the same centre and size) and each says where it is through `capture()`; every speck is paired with **the nearest speck of the other** on that sphere, and the whole turns a part-turn out and back on the way, so each lands exactly on its twin. The new drawing is **held** (`hold()`) at the frame the flight is aimed at until it lands; the old one, glow and all, fades into its own specks over the first fifth. `morph`, `MORPH_*` in `chamber.js`. It used to be a cut, then cut at both ends, and until 2026-09-25 a cloud that swept across the window between two different places — "choppy because they are in different places". **Smoothed** the night of 2026-09-25 ("quite laggy"): a held drawing draws once rather than every frame under its veil, the flight's specks are drawn in batches of one colour and brightness, the blooms under the brightest 500 only, and the setting up waits for the frame after the press, which is answered at once. |
| **the spin-up** (a chapter) | What a chapter's drawing does once the morph has landed and it is let go: rather than setting off at full speed, its clock runs at a rate rising from nothing to full over `SPIN_UP_MS` (1.4s), smoothly at both ends — the owner's "pick up speed and accelerate into the speed that it is currently spinning at. make that SLIGHTLY gradual". In `sun.js` and `moon.js`. |
...
| **opening a favourite** | What clicking a card does: every card stays where it is and **the drawer** opens under its row, on a measured height, in two columns — on the left the favourite's name and house, the owner's own words about it (its block on this page), and two plain links, **Notes** (the site's own notes window in this page's colours) and then **Read the whole entry** (the entry's own `href`) — the other way round until 2026-09-26; on the right **the print**. One at a time. Until 2026-09-26 the card took the whole width of the grid where it stood, which from the second or third column left the cards before it alone on a short row — "too techy and leaves an awkward space" — and read **the start of its own entry** off the page it lives on where the owner had written nothing; both are gone. The card is a `<button>` and not a link, because what it opens carries links; it is opened by a drawn **sign**, a plus in a circle that turns to a minus. It said OPEN ↓ / CLOSE ↑ and GO TO FRAGRANCE in capitals until 2026-09-26 — "less techy, more minimalist and geometric". |
| **the drawer** (a favourite) | The panel an opened favourite stands in (`.fav-drawer`): the full width of the chapter's grid, opened **under the row** of the card pressed — after the row's last card, counted off the grid's own columns — with the rows after it going down by its height and the card itself not moving; a short **stem** let down from the open card to its top rule, a close top right, and the page carried up to show it when it opens low on the window (`reveal`). Another card in the same row **swaps** what it carries; a card in another row moves it there. What it carries is the card's own body, moved in and out rather than copied. `chamber.js` (`openDrawer`, `swapInto`, `shutDrawer`). |
| **the print** (a favourite) | The favourite's picture in an opened card (in **the drawer**): an upright frame (4 : 5) the picture **fills edge to edge**, nothing behind it, **placed on the bottle** — the bottle found in the picture itself, the picture brought closer where the bottle is small — or, on a clean white or black ground, drawn back until the whole bottle fits, the frame taking that colour — and moved so it stands in the middle (`seat`, `readPicture`, `placeIn` in `chamber.js`, the same reading as the Fragrances view's; a border printed into a picture is trimmed off). Credited under it. `.fav-print`. (Not the chamber's own **plate**, the word and its menu.) |
| **the circle** (a favourite) | **Removed** (2026-09-26, the same night it came): the picture whole in a circle over a blurred copy of itself, a studio shot multiplied onto a pale disc, a scene feathered into its blur, and a glowing ring of the sun's round it — "it looks tacky". No `.fav-disc`, `is-studio`, `is-scene` or `RING_*` anywhere. |
| **the sun** | The ground behind Chapter 1: a sphere far bigger than the window drawn entirely in specks — a Fibonacci surface, three latitude rings and four meridians turning with it, a **registration ring** that does not turn, five **prominences** rising off the limb, and a corona. `sun.js`. Its readability is Ataraxia's, by name and by the same two mechanisms at its own values: a speck over the sheet is drawn at `QUIET` of its strength and its **bloom** is scaled by `hush³`, so the glow is gone long before the writing is. **It answers an opened favourite** (`attend`, since 2026-09-26): a fine **line** of its own specks run round the favourite's **print** a little outside it, lit from the sun's side and drawn past the quiet, a **registration mark** at each corner, and the sun's own surface round it a little brighter (`HALO`; not either removed **halo**), turning towards it over a second and away in half of one. It was a glowing ring with ticks round the removed **circle** for one round. |
| **the moon** | The ground behind Chapter 2, since 2026-09-24: the sun's construction turned to night, on **the sun's own tilt and rate, the same way round** (`TILT`, `SPIN`, the same numbers in both files), and since 2026-09-25 **exactly where the sun stands and exactly as big** (`AT_X`, `AT_Y`, `BIG`) — it stood a little lower and smaller. Lit on one side with a soft terminator and earthshine, the light swinging from full to a thick crescent and back (**the phases**, never a new moon); **craters** with rims, peaks and rays, dark **maria**, and a tilted **ring of debris** turning faster than it. `moon.js`. |
...
| **the swipe** | How the contact sheet's two views change over **once both have been opened** — but only while the Fragrances view is the old table (its script blocked); otherwise it is **the crossing**. The page travels sideways, what you are leaving going off one edge as what you are going to comes in from the other. The first time a view is opened there is no swipe — it is the plain swap. The chrome does not travel: the Menu, the category's name, the buttons and the search all live outside the box that slides. `views.js`. |
| **the crossing** / **the divider** (views) | How the Houses view and the Fragrances change over since the night of 2026-09-25 — the owner's "something simple, so that you can freely change between the houses and fragrances page. Make the two pages connected somehow too": the one being left fades out drifting a little the way it is going as the other fades in from the other side, both at once, in 380ms (`cross()`, `CROSS_MS`, `CROSS_SHIFT` in `views.js`). **Free**: a press while it runs turns it round at once. **Connected** by one hairline (`.views-thread`, in the views' box) that stands where the houses' axis stands and travels to **the divider** — the hairline between the table's aside and its items (`.frag-divider`, its x published as `data-divider`) — and back; and each view carries the way to the other (`data-view-go`: *The fragrances →* under the way round, *← The houses* at the foot of the aside). Coming back, the line lands **exactly on the axis** — where it stands at rest, in the middle of the page — and goes there (it used to land short, reading the axis while its view was still drifting in). None with reduced motion. (Not the landing page's **thread**.) |
| **the stretch** | **Removed** (night of 2026-09-25) for **the crossing**: for most of that day the Houses' axis was smeared to the right in pixel streaks, the page travelled a window, and the streaks gathered onto the table's rules. Nothing of it is in the code — no `stretch()`, no `.view-stretch`, no `data-rules`. If the owner says the stretch or the pixel stretch, they mean that. |
| **view** | One of the two ways the Scent descriptions page shows its category, behind the two buttons across the top: the **houses** (the axis) and the **fragrances** (the whole-page table; the index underneath it). `views.js` switches them, and only one is ever on the page except during the crossing (or the swipe). It briefly had a different pair — the **map** and **Favorites**, the second of which was the removed **register** — so if the owner says "Description portfolio" or "the Favorites view", they mean those. |
| **houses** | The Houses view: one picture per house, on the **helix**, in order — 01 at the front to begin with. |
| **the axis** (houses) | How the Houses view is laid out since the evening of 2026-09-24 — the owner's "something to do with aprticles ... a central axis ... impressive, and navigatable": a line down the middle of the page with a faint light either side and pulses running down it, specks always falling down it, ruled with ticks that travel as you do, carrying each house's number as a button where that house stands. It runs **from the very top of the window to its very foot**, up between the two words **Houses** and **Fragrances**, which stand either side of it — the owner's "all the way up" and "all the way down"; and a page arriving draws it **first**, then the helix, then the houses in number order. `contact-sheet.js`. (A hairline until the owner asked for it emphasised; then, the night of 2026-09-25, **quieter** again — "de emphasize the blacj line in the middle ... (especially with the shadow around it)": `AXIS_LINE`, `AXIS_LINE_LIT`, `AXIS_GLOW`, `AXIS_GLOW_LIT`, `PULSE_LIT`.) |
| **the afterglow** (houses) | What the Houses view's specks do once the pointer has passed over them: they stay lit a moment (`GLOW_HOLD`) and then go out (`GLOW_FADE`), lit by a **trail** of where the pointer has been. The owner's "a delay of the particles turning off after you hover them" — done on Ataraxia's kindle as well, since the note did not say which page. |
...
| **the front** (houses) | The house on the axis, facing you — the one the page **rests** on: the largest and the only one at full strength — the ones either side about two thirds of its size and strength (`SIDE_FALL`, `SIDE_FADE`), the ones beyond under a third as strong, since the owner asked for the others "slightly less prioritized" (2026-09-25) — marked by **the corners**. Pressing it opens the house; pressing any other brings that one to the front. |
| **the corners** (houses) | Four brackets of specks standing just outside the front house **and its label**, a light running out along their arms — a registration mark round what it registers. They replaced **the halo**. `CORNER_*` in `contact-sheet.js`. |
| **the halo** (houses) | **Removed** (2026-09-24): a ring of specks turning round the front house, which ran under the picture's corners and was mostly hidden — the owner's "spinning particle circle behind the house". Nothing of it (`HALO_SPECKS`) is in the code. (Not Ataraxia's removed halo, above.) |
| **spoke** (houses) | What joins each house to the axis since the night of 2026-09-25 — the owner's "connect the fragrances to the spiral in a 3d way ... make the connecting line particular": a loose tube of particles (`SPOKE_PARTS`) standing in the helix's own space from the axis out to its house, turning with the house as the helix turns, nearer particles larger, flowing outward, the tube twisting slowly about itself, a pulse running out along it; the half behind the axis drawn behind it (`spokesPass`). A house arriving comes out along its spoke. |
| **tether** | **Removed** (2026-09-25): the flat line of specks from the axis straight out to a house, lying on the window, with a pulse running outward along it. The **spoke** replaced it. |
...
| **motifs** | What comes up over the Houses view while a house is **rested on**: that house's own things, taken from its page — Pineward's trees and needles, ADAR's **wells** — black holes whose waves and pull bend the whole view — and dust, Almost Human's **clouded figures** glitching into being and standing, and rain, Ataraxia's bands of plain particles **running as a wave** and **shaking the air** round them (their shadows and before that a grey haze were taken out), Grande's particles **rising and bursting** into finer ones (they were bubbles for a round), Les Abstraits' **armoire** in lines with clothes inside and iris growing from a tuft at each front leg, and a slow **drip** into a **beaker** that overflows (it was a logo made of droplets, and before that smoke, compositions, and a point, a line and a circle), Tale's doodles, Tombstone's **roots** creeping in — from the very top edge too — and red flowers opening, whose **petals fall and gather** along the foot of the window until the house is left, and Qimu & Musicians' **music** — **short** staves written out left to right, with real and changing time signatures and no dynamics or ornaments, taking turns with **loose music**. They gather one at a time and **fade** when the house is left, never vanishing, and they stand **behind the houses**, over the whole page. Tombstone **writes nothing** since 2026-09-25 — its **epitaphs**, its five names cut into the wall and worn away, were taken out at the owner's word. `motifs.js`, named per frame by `data-motif`. |
| **the epitaphs** (Tombstone's motif) | **Removed** (2026-09-25, last): the house's five names cut into the wall letter by letter while Tombstone was rested on, and worn away — each once, kept off the houses and 40px from each other. The owner: "Remove the names of the fragrances that pop up with the tombstone hover". Nothing of it is in `motifs.js` (no `epitaph`, `EPITAPHS`, `NAME_APART`); the roots, the soil and the red flowers are what is left. |
| **the concentrate** (Les Abstraits) | **Removed** (2026-09-25): Les Abstraits' motif for one round — droplets drawing in onto the house's own logo, which then set solid. The owner did not want it "to ever turn into the actual picture". Nothing in the code reads the logo now. |
| **the armoire** / **the drip** / **the beaker** / **the spill** / **the puddle** (Les Abstraits) | Les Abstraits' ground on its own page (`abstraits.js`) and its motif on the Houses view (`armoire()`, `drip()`): an old armoire building up from the floor, its door ajar with **orris powder** drifting out — Belle Âme, at the owner's word. On the page it is drawn in walnut specks with **three irises** inside the door, under a shelf of folded clothes and a rail of hung ones; **on the Houses view, since the night of 2026-09-25, it is geometry in walnut hairlines** on four tapered legs, a speck only at each joint, the inside seen in perspective through the door, and the irises **growing in a fan at its front legs** — sword leaves, then stems, then three falls and three standards, one bud shut — the owner's "less particular dense, and more geometric ... coming out from the legs of it, like real flowers would". The irises there grow from **a tuft** — every leaf its own, some flopped or browned, soil and grass round the crown — and through its open door there are **clothes**: a coat, a dress and a shirt on wire hangers from a rail, and folded stacks on the shelf and the floor of it. The page's own armoire has them too, in specks, with its irises under the shelf. On the other side a bead gathers at the very top and falls, slowly, into **the beaker** (since the night of 2026-09-25; it was **the puddle**, a plain ellipse growing to `PUDDLE_MOST`, which is gone): drawn by `beaker.js` for both, **as a diagram** — straight walls in even hairlines, a spout, graduations to 250 ml — filling with every drop (half as fast since 2026-09-25: `BEAKER_FILL` 24, `FILL_DROPS` 32; a **pointer** at the surface with its reading in ml stood outside the wall for a round and is gone) and then **overflowing** — a run down its outside and a bead dropping from the spout onto **the spill**, a flat lens with rings, on a hatched **bench**. On the house's own page the drip runs **the whole length of the page**, carried and streaked by the scroll, to a beaker at the page's foot; on the Houses view it falls the height of the window. |
| **loose music** (Qimu) | Passages with no staff — a run under its tuplet, a flurry of small notes under a beam, a cluster, a few chords — popping up on the Houses view while Qimu is rested on, **taking turns** with the staves: nine seconds of staves, six of loose music. No dynamics, trills, fermatas or other expression marks since 2026-09-25. `passage()`, `QIMU_*` in `motifs.js`. |
| **the score** (Qimu) | Qimu & Musicians' ground on its own page (`qimu.js`): short staves down both margins, carried with the page, written in as they are reached and then **played** — a faint **playhead** along each, bar by bar, the notes lifting as it passes — with real, changing time signatures, on semi-light blue paper. |
| **the stones** / **the reflection** (Tombstone) | Tombstone's ground (`tombstone.js`): headstones, a cross and an obelisk made of specks standing on a still grey floor in the margins, each **gathering** out of the mist, standing, and **going** as smoke goes, with its **reflection** turned over below the horizon, streaked and trembling, and ringing under the pointer; mist along the floor and ash falling. A first answer to "very dead and funerary ... a reflection ... ephermeral"; the owner said they would think of the particulars later. |
| **the wave** / **the air** (Ataraxia's motif) | What Ataraxia's bands do on the Houses view since 2026-09-25: a ripple running along each band that its specks ride, swelling where a crest passes, and a looser scatter of fainter specks either side — the air — trembling with it, hardest near the band and the crest. `WAVE_*`, `SHAKE_REACH` in `motifs.js`. (Not the page's own **crest**, which is light.) |
| **the glitch** (Almost Human's motif) | How a figure comes over the Houses view since 2026-09-25: a **clouded figure** — volumes filled with specks thrown softly off their places, a person seen through frosted glass — arriving as a broken signal finds itself: **stutter**, a red and a cyan **colour split** closing on it, a **smear** across a band of it, **interlace**; then standing; then going as an old screen turned off does, pressed to a line and a point. `FIG_*` in `motifs.js`. The band-slicing glitch before it (`GLITCH_*`) the owner called "mid", and it is gone. (Not the house page's **fault**.) |
...
| **fragrances** (the view) | The second view of the contact sheet page. Since the night of 2026-09-25 it is **the table** with its aside and its three layouts — see below (it was **the line** for a day) — with the old **index** kept underneath it, exactly, and in `archive/`. It **used to list every fragrance on the whole site** and point back into the houses; since 2026-09-21 it carries only the perfumes that belong to no house, opening into `individual-fragrances/individual-fragrances.html`. |
| **the table** / **the aside** / **the scale** / **the mark** (fragrances) | The Fragrances view since the night of 2026-09-25 (`fragrance-line.js`, `.frag-stage`): **the table** nearly the whole page — number in three digits, name, house, date — sortable and searched, scrolling in its own box (the wheel scrolls it from anywhere), each item opening its fragrance in the page. **The aside** on its left: its name, **Individual** set small over **Fragrances** (it said *The ones with no house here* under the name until 2026-09-25), **the count** large, **readings** (houses, last written, sorted by, shown as), **the scale** — a hairline with a tick for every fragrance shown, filled as far as has been seen — and **the mark**, a small ring of specks turning slowly with one larger speck for every fragrance, the one pointed at boxed, and at its foot *← The houses*. **The divider**, a hairline between the aside and the table, is where the crossing's line lands. Nothing is drawn behind it. |
| **list** / **boxes** / **cards** (fragrances) | The three ways of showing that table, chosen by the three buttons on its right (`.frag-options`, `is-list` / `is-boxes` / `is-cards` on the stage): ruled rows; small squares, several to a row; larger cards with each fragrance's picture, fetched from `individual-fragrances.html` the first time, the hatching and the number where the picture has not arrived. **Since 2026-09-26 the boxes carry the pictures too** — a box is its picture, square, over a small label — and a card's picture is upright (4 : 5); in both **the picture fills its frame edge to edge with nothing behind it**, placed so the bottle is in the middle and brought closer where it is small — or, on a clean white or black ground, drawn back until the whole bottle fits, the frame taking that colour (`seat`, `readPicture`, `placeIn` in `fragrance-line.js`). For one round it stood whole over a blurred copy of itself, which the owner did not want. The choice is kept for the next visit in that browser. |
...
| **favourite** | One entry in Favorites (`<a class="gallery-entry">`), carrying a `data-chapter`, a `data-house`, optionally a `data-notes` key into `notes-data.js`, a `data-image` with its `data-credit` and `data-credit-href`, and an `href` pointing at wherever that fragrance lives on the site. It carried a `data-date` until 2026-09-22; if the owner uses the word, that is what it was. |
...
- **There are nine houses now** (Pineward, ADAR, Almost Human, Ataraxia, Grande Parfums,
  ...
  writing and a drawing that **deliberately says nothing about the house** (its specks
  burst now, as the owner asked, on champagne paper), because the owner has still not
  said what it is; Les Abstraits has its names, its notes, its writing and — since
  2026-09-25 — its armoire and its drip, on iris paper; Tale Parfums has its writing, its
  pictures and its hand-drawn page and **no notes yet**, because its own site could not be
  read to take them from; Tombstone is written but for the rest of 3 Feet 5, and since
  2026-09-25 has its stones and their reflections — a first answer; the owner will give
  the particulars; and Qimu & Musicians has Guitarist and Vocal written, Drummer saying
  "Description coming soon.", Bassist and the introduction waiting, notes for only one,
  and — since 2026-09-25 — its score in the margins on semi-light blue. **Named is not written**, and since 2026-09-22 the site has
...
were taken off every house at the owner's word), and every
