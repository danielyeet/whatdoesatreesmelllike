...
| `categories/note-library.html` | the **Note Library**: every note named in a fragrance on the site, filed by **accord** and explained — **books** standing on bare **boards** at night, each spine drawn in **flat geometric shapes** in the cloth of its accord with a library **label** at its foot carrying its **call number**, read through a catalogue **terminal**, each book as thick as the note is used, opening into a **catalogue card** of hairlines and specks, with **the mark** at its head, that links every fragrance using it | `search.js`, `notes-data.js`, `note-library.js` | [the Note Library](docs/features/2026-09-24-the-note-library.md) |
...
`search-page.js` (~200), `photography.js` (~190), `find-ground.js` (~180)
...
**A clean run is 384 passed, 0 failed, and takes seven to twenty minutes.** If you get a
...
- **`prefers-reduced-motion: reduce`** — read by `landing.js`, `paper.js`, `thread.js`,
  ...
  `note-library.js`, `index-page.js`, `fragrance-reader.js`, `fragrance-line.js` (the mark
...
| **accord** (library) / **shelf** | One family of notes in the Note Library — Citrus, Woods, Impressions and so on — with a three-letter **code** (`CIT`, `WOO`, `IMP`), a colour of its own, and its books standing in **a case** of their own on rows that wrap across the page. The page calls it an **accord** since the owner asked ("Replace the word shelves with accords"); the markup and the code still say shelf (`.lib-shelf`, `data-shelf`), which nobody reads. (Not the primer's **accords table**, which is perfumery's own sense of the word.) |
...
| **book** / **spine** / **cloth** (library) | A record as it stands on its shelf: a **book** as thick as the number of fragrances that use it, as tall as its own (seeded), its name down its **spine**, which is drawn in **flat geometric shapes** on a canvas of its own (`drawSpine`, `.lib-spine`) in the **cloth** its accord is bound in (`CLOTH` — citrus an ochre, the herbs a sage, the woods a walnut, the airs a slate blue): a darker edge either side and a lit face, a cap and a tail, on some a title panel with a gilt edge or a stripe, raised bands at head and foot, on some a gilt rule, and a **label** at its foot. It was **drawn in specks** for a round (2026-09-25) until the owner found the books "granular" and asked for "more of a geometric character". The last on each shelf **leans**. Pressing one **pulls it out** and opens its card. They were **folders** — graphite files coloured only on a **tab**, then "digital" — for two rounds (2026-09-24), and books before that; if the owner says folder or tab, they mean that. |
| **call number** / **the label** (library) | A book's accord code and its place in it, counted alphabetically — `WOO 007`, printed in dark ink on **the label**, a small square of cream specks at the foot of its spine as a library book carries one. Worked out, not written, and unchanged when the books are reordered by use. |
| **the boards** / **the case** / **the label holder** (library) | What an accord's books stand on: since 2026-09-25, last, **a board** under every row and **nothing round it** (`canvas.lib-boards`, `drawBoards`, `BOARD_*`, `WOOD`) — flat walnut, its top lit, a lit arris, its front a shade darker, a soft shadow under it, running `BOARD_OUT` past the row either side; on each board's front a cream card giving the call numbers on that row, `CIT 001–013`. Drawn when it comes near the window and again when the rows change. **The case** — a bookcase of walnut **drawn in specks** round every accord, two uprights, a crown, a plinth, a back of boards, and a brass **label holder** round each card — stood for one round (the night of 2026-09-25) and is **removed**: the owner's "remove the bezel of the bookshelves"; no `drawCase`, no `.lib-case`. Before the case it was **a rail** — one lit line under each row, a gradient on `.lib-records` — also gone. (Not a book's own **label**, above.) |
...
| **the lamp** / **the dust** (library) | The Note Library's room: a soft light following the pointer over the stacks **a beat behind it**, as the cursor's square does (`LAMP_LAG`, the cursor's own 0.16), and specks in the air brighter near it. The scan that passed down the window now and then was taken out with the glass (2026-09-25). |
