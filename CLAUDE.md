...
| `categories/note-library.html` | the **Note Library**: every note named in a fragrance on the site, filed by **accord** and explained — in **black and white**, the **books** drawn as hairline **skeletons** on hairline **boards**, each with its accord's colour in one band across its head and a library **label** at its foot carrying its **call number**, read through a catalogue **terminal**, each book as thick as the note is used, opening into a **catalogue card** of hairlines and specks, with **the mark** at its head, that links every fragrance using it | `search.js`, `notes-data.js`, `note-library.js` | [the Note Library](docs/features/2026-09-24-the-note-library.md) |
...
`note-library.js` (~1,220), `almost-human.js` (~1,170), `contact-sheet.js` (~1,000),
...
**A clean run is 391 passed, 0 failed, and takes seven to twenty minutes.** If you get a
...
| **record** (library) | One note in the Note Library: an `<article class="lib-record">` with its name, the other spellings folded into it (`data-aka`), and what it is (`.lib-say`). Stood up as a **folder**. |
| **accord** (library) / **shelf** | One family of notes in the Note Library — Citrus, Woods, Impressions and so on — with a three-letter **code** (`CIT`, `WOO`, `IMP`), a colour of its own (the only colour on the page since it went black and white, 2026-09-26: on its tab, its code, the band across each of its books' heads and its card's specks), and its books standing on boards of their own on rows that wrap across the page. The page calls it an **accord** since the owner asked ("Replace the word shelves with accords"); the markup and the code still say shelf (`.lib-shelf`, `data-shelf`), which nobody reads. (Not the primer's **accords table**, which is perfumery's own sense of the word.) |
...
| **book** / **spine** / **cloth** (library) | A record as it stands on its shelf: a **book** as thick as the number of fragrances that use it, as tall as its own (seeded), its name down its **spine**, which is drawn as **a skeleton** since 2026-09-26 — in the page's white, in hairlines, on a canvas of its own (`drawSpine`, `.lib-spine`): its outline, two bands near head and foot (some doubled), on some a title panel or a rule down one side, and a hairline **label** at its foot — with **its accord's colour in one place only, the band across its head** (`markColour`, the same colour as the accord's tab): the owner's "a minimalist approach with the colour coding of the books ... more skeletal/geometric". **The cloth is removed**: for two rounds the spine was its accord's cloth from head to foot (`CLOTH` — citrus an ochre, the herbs a sage, the woods a walnut), with gilt — in flat shapes (2026-09-25, last), and before that **drawn in specks**, until the owner found the books "granular" and asked for "more of a geometric character". The last on each shelf **leans**. Pressing one **pulls it out** and opens its card. They were **folders** — graphite files coloured only on a **tab**, then "digital" — for two rounds (2026-09-24), and books before that; if the owner says folder or tab, they mean that. |
| **call number** / **the label** (library) | A book's accord code and its place in it, counted alphabetically — `WOO 007`, printed in white in **the label**, a hairline box at the foot of its spine as a library book carries one (a cream one until the page went black and white). Worked out, not written, and unchanged when the books are reordered by use. |
| **the boards** / **the case** / **the label holder** (library) | What an accord's books stand on: since 2026-09-25, last, **a board** under every row and **nothing round it** (`canvas.lib-boards`, `drawBoards`, `BOARD_*`) — since 2026-09-26 **drawn in hairlines** as a plank in a diagram is: its back edge, its arris, the foot of its front, both ends closed, a ruler of ticks along its foot, running `BOARD_OUT` past the row either side; on each board's front a hairline box giving the call numbers on that row, `CIT 001–013`. For its first round it was flat **walnut** (`WOOD`), lit and shadowed, with a cream card; that went with the colour. Drawn when it comes near the window and again when the rows change. **The case** — a bookcase of walnut **drawn in specks** round every accord, two uprights, a crown, a plinth, a back of boards, and a brass **label holder** round each card — stood for one round (the night of 2026-09-25) and is **removed**: the owner's "remove the bezel of the bookshelves"; no `drawCase`, no `.lib-case`. Before the case it was **a rail** — one lit line under each row, a gradient on `.lib-records` — also gone. (Not a book's own **label**, above.) |
...
| **the lamp** / **the dust** (library) | The Note Library's room: a soft white light following the pointer over the stacks **a beat behind it**, as the cursor's square does (`LAMP_LAG`, the cursor's own 0.16), and specks in the air brighter near it. The scan that passed down the window now and then was taken out with the glass (2026-09-25). |
