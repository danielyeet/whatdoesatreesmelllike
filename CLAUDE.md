...
| `houses/les-abstraits.html` | **Les Abstraits**, the sixth house: four fragrances, written, all four with notes off the house's own page — with a **drawing** of Des Cendres' road standing in its writing, and a **last word** at the foot whose link opens a new window; on iris paper, with an **armoire** of specks in the left margin, iris in it, and a **drip** down the right margin the whole length of the page into a **beaker** at its foot, which overflows | `search.js`, `house.js`, `abstraits.js`, `notes-data.js`, `notes.js` | [the newer houses](docs/features/2026-09-21-the-newer-houses.md) |
...
Three of those page scripts are elaborate: `chamber.js` (~3,540 lines), `structure.js`
(~1,580) and `node-scene.js` (~1,530). The rest are smaller:
`motifs.js` (~1,520), `almost-human.js` (~1,170), `note-library.js` (~1,100),
`pineward.js` (~970), `contact-sheet.js` (~900), `adar.js` (~890),
`calculator.js` (~780), `sun.js` (~650), `fragrance-reader.js` (~640), `tale.js` (~590),
`moon.js` (~590), `paper.js` (~580), `fragrance-line.js` (~580), `notes.js` (~490),
`views.js` (~470), `qimu.js` (~460), `essay.js` (~440), `ataraxia.js` (~410),
`abstraits.js` (~360), `house.js` (~350), `pineward-gallery.js` (~350), `tombstone.js`
(~330), `index-page.js` (~310), `grande.js` (~300), `nav.js` (~290), `thread.js` (~290),
`search.js` (~290), `extras.js` (~250), `primer.js` (~230), `landing.js` (~230),
`search-page.js` (~200), `photography.js` (~190), `find-ground.js` (~180) and
`page-search.js` (~110).
...
**A clean run is 358 passed, 0 failed, and takes seven to seventeen minutes.** If you get a
...
- **`prefers-reduced-motion: reduce`** — read by `landing.js`, `paper.js`, `thread.js`,
  ...
  `views.js` (no stretch, no swipe, no fade — the views simply change over) and
...
| feature | file | report |
|---|---|---|
| The Fragrances view as a whole-page table with three ways of showing it, and the stretch from the Houses | `fragrance-line.js`, `views.js` | [report](docs/features/2026-09-25-the-fragrance-line.md) |
...
| **accord** (library) / **shelf** | One family of notes in the Note Library — Citrus, Woods, Impressions and so on — with a three-letter **code** (`CIT`, `WOO`, `IMP`), a colour of its own, and its folders standing on rows that wrap across the page. The page calls it an **accord** since the owner asked ("Replace the word shelves with accords"); the markup and the code still say shelf (`.lib-shelf`, `data-shelf`), which nobody reads. (Not the primer's **accords table**, which is perfumery's own sense of the word.) |
...
| **the swipe** | How the contact sheet's two views change over **once both have been opened** — but only while the Fragrances view is the old table (its line blocked); with the line it is **the stretch**. The page travels sideways, what you are leaving going off one edge as what you are going to comes in from the other. The first time a view is opened there is no swipe — it is the plain swap. The chrome does not travel: the Menu, the category's name, the buttons and the search all live outside the box that slides. `views.js`. |
| **the stretch** | How the Houses view becomes the Fragrances (and back) since 2026-09-25: everything but the particles fades, the axis is **smeared to the right** as a column of pixels is when it is stretched — a streak for every few pixels down it, each its own weight and reach — the page travels a window left so the streaks run from the left edge to the middle, and there they **gather** onto the table's own **rules** — the rule under its sort bar and the line under every row in the window, which the view publishes as `data-rules` — pulled in to the table's width; then what is written in the table comes in. (Until the night of 2026-09-25 they gathered into a line across the window, which the owner had taken out.) Backwards going back. Minimal and geometric, at the owner's word. `stretch()` in `views.js`, `.view-stretch`; the Fragrances view is told to wait and then that its ruling is drawn through `data-arrive` (`"wait"`, `"ruled"`). None with reduced motion. |
| **view** | One of the two ways the Scent descriptions page shows its category, behind the two buttons across the top: the **houses** (the axis) and the **fragrances** (the whole-page table; the index underneath it). `views.js` switches them, and only one is ever on the page except during the stretch (or the swipe). It briefly had a different pair — the **map** and **Favorites**, the second of which was the removed **register** — so if the owner says "Description portfolio" or "the Favorites view", they mean those. |
