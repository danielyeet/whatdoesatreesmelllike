...
| `categories/note-library.html` | the **Note Library**: every note named in a fragrance on the site, filed by **accord** and explained — **books** on shelves at night, each spine **drawn in specks** in the cloth of its accord with a library **label** at its foot carrying its **call number**, read through a catalogue **terminal**, each book as thick as the note is used, opening into a **catalogue card** of hairlines and specks, with **the mark** at its head, that links every fragrance using it | `search.js`, `notes-data.js`, `note-library.js` | [the Note Library](docs/features/2026-09-24-the-note-library.md) |
...
| `houses/grande-parfums.html` | **Grande Parfums**, the fifth house: seventeen fragrances — fifteen written up alphabetically, two at the foot not smelled yet — over a **drift** of fine specks rising slowly up the window and **bursting** into finer ones on the way, on champagne paper, which claims no theme because the owner has not given one | `search.js`, `house.js`, `grande.js` | [the newer houses](docs/features/2026-09-21-the-newer-houses.md) |
...
(~2,190), `structure.js` (~1,580) and `node-scene.js` (~1,530). The rest are smaller:
`note-library.js` (~1,300), `almost-human.js` (~1,170),
`pineward.js` (~970), `contact-sheet.js` (~950), `adar.js` (~890),
`calculator.js` (~780), `sun.js` (~680), `fragrance-reader.js` (~640), `moon.js` (~620),
`tale.js` (~590), `paper.js` (~580), `fragrance-line.js` (~560), `notes.js` (~490),
`qimu.js` (~460), `essay.js` (~440), `ataraxia.js` (~410), `abstraits.js` (~400),
`views.js` (~390), `house.js` (~350), `pineward-gallery.js` (~350), `tombstone.js`
(~330), `index-page.js` (~310), `grande.js` (~300), `nav.js` (~290), `thread.js` (~290),
`search.js` (~290), `extras.js` (~250), `primer.js` (~230), `landing.js` (~230),
`beaker.js` (~260), `search-page.js` (~200), `photography.js` (~190), `find-ground.js` (~180)
...
**A clean run is 364 passed, 0 failed, and takes seven to seventeen minutes.** If you get a
...
| **the drift** (Grande Parfums) | That page's ground, and the quietest thing on the site: fine specks rising slowly up the window, each fading in and out on a clock of its own, with a few larger **motes** among them; they **lean** towards the pointer rather than being pulled to it. Since 2026-09-25 about half of them (and every mote) **burst** on the way up into a spray of finer specks — the owner's "particles that rise up and pop more or less into a bunch of other smaller particles", and never drawn bubbles. On champagne paper. `grande.js`, `BURST_*`. |
...
| **book** / **spine** / **cloth** (library) | A record as it stands on its shelf: a **book** as thick as the number of fragrances that use it, as tall as its own (seeded), its name down its **spine**, which is **drawn in specks** on a canvas of its own (`drawSpine`, `.lib-spine`) in the **cloth** its accord is bound in (`CLOTH` — citrus an ochre, the herbs a sage, the woods a walnut, the airs a slate blue), rounded by the light, with raised bands at head and foot and on some a gilt rule, and a **label** at its foot. The last on each shelf **leans**. Pressing one **pulls it out** and opens its card. They were **folders** — graphite files coloured only on a **tab**, then "digital" — for two rounds (2026-09-24), and books before that; if the owner says folder or tab, they mean that. |
...
| **the case** / **the board** / **the label holder** (library) | What an accord's books stand in since the night of 2026-09-25 — the owner's "redesign the shelves": a bookcase of dark walnut **drawn in specks** behind the books (`canvas.lib-case`, `drawCase`, `WOOD`) — two uprights, a crown, a plinth, a back of boards with their seams, and **a board** under every row, its top lit and its front with a lit arris; on each board's front a brass **label holder** with a card giving the call numbers on that row, `CIT 001–013`. Drawn when it comes near the window and again when the rows change. It was **a rail** — one lit line under each row, a gradient on `.lib-records` — which is gone. (Not a book's own **label**, above.) |
...
| **the front** (houses) | The house on the axis, facing you: the largest — the ones either side about three quarters of it (`SIDE_FALL`) — marked by **the corners**. Pressing it opens the house; pressing any other brings that one to the front. |
...
| **the table** / **the aside** / **the scale** / **the mark** (fragrances) | The Fragrances view since the night of 2026-09-25 (`fragrance-line.js`, `.frag-stage`): **the table** nearly the whole page — number in three digits, name, house, date — sortable and searched, scrolling in its own box (the wheel scrolls it from anywhere), each item opening its fragrance in the page. **The aside** on its left: the name, **the count** large, **readings** (houses, last written, sorted by, shown as), **the scale** — a hairline with a tick for every fragrance shown, filled as far as has been seen — and **the mark**, a small ring of specks turning slowly with one larger speck for every fragrance, the one pointed at boxed, and at its foot *← The houses*. **The divider**, a hairline between the aside and the table, is where the crossing's line lands. Nothing is drawn behind it. |
...
and most of the site's plates are waiting for the owner (the standfirsts that waited too
