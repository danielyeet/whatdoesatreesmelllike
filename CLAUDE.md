...
`almost-human.js` (~1,170), `note-library.js` (~990), `pineward.js` (~930),
`contact-sheet.js` (~900, down from ~1,570 when the chain replaced the map, and up
again as the hang and then the axis), `motifs.js` (~900), `adar.js` (~890),
...
**A clean run is 333 passed, 0 failed, and takes seven to fifteen minutes.** If you get a
...
| **motifs** | What comes up over the Houses view while a house is **rested on**: that house's own things, taken from its page — Pineward's trees and needles, ADAR's void and dust, Almost Human's figures and rain, Ataraxia's bands (strengthened at the owner's word), Grande's drift, Les Abstraits' **point, line and circle** — one amber point, a horizon drawn out from it, and one great circle round it in a single brush stroke that runs dry, an *ensō* — the owner's "more profound and yet minimalist" (it was smoke and embers for two rounds, then abstract compositions of circles, arcs, triangles and dots for two more), Tale's doodles, Tombstone's **epitaphs** (its five names cut into the wall letter by letter and worn away) with **roots** creeping in — from the very top edge too — and red flowers opening, whose **petals fall and gather** in a drift along the foot of the window until the house is left, and Qimu & Musicians' **staves** — five lines drawn across the page with notes coming and going on them. They gather one at a time and **fade** when the house is left, never vanishing, and they stand **behind the houses**, over the whole page. Ataraxia's bands are **fewer and bolder** since the last round. Tombstone writes each of its five names **once**, kept off the houses and their labels and at least 40px from each other; Qimu's are quiet and hardly move. `motifs.js`, named per frame by `data-motif`. Tombstone's were stones in mist for one round, and Qimu's notes, records and a line of sound. |
