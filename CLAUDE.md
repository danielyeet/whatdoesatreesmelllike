...
`moon.js` (~590), `paper.js` (~580), `fragrance-line.js` (~540), `notes.js` (~490),
...
**A clean run is 337 passed, 0 failed, and takes seven to fifteen minutes.** If you get a
...
- **`prefers-reduced-motion: reduce`** — read by `landing.js`, `paper.js`, `thread.js`,
  ...
  `note-library.js`, `index-page.js`, `fragrance-reader.js`, `fragrance-line.js`,
...
| feature | file | report |
|---|---|---|
| The Fragrances view as a line of files round a double pyramid, and the stretch from the Houses | `fragrance-line.js`, `views.js` | [report](docs/features/2026-09-25-the-fragrance-line.md) |
...
- **New piece of work**: duplicate a template in `works/` — `example-gallery-work.html`
  ...
  another **file** on the line (`fragrance-line.js` reads the table). Each page says which in the comment at the top
  of it.
...
| **the armoire** / **the drip** / **the puddle** (Les Abstraits) | Les Abstraits' ground on its own page (`abstraits.js`) and its motif on the Houses view (`armoire()`, `drip()`): an old armoire drawn in walnut specks, building up from the floor, its door ajar on **three irises** with **orris powder** drifting out — Belle Âme, at the owner's word — and on the other side a bead gathering at the very top of the window and falling into **the puddle**, which starts as nothing and grows with every drop to a size it stops at (`PUDDLE_MOST`). |
...
| **fragrances** (the view) | The second view of the contact sheet page. Since 2026-09-25 it is **the line** — see below — with the old **index** kept underneath it, exactly, and in `archive/`. It **used to list every fragrance on the whole site** and point back into the houses; since 2026-09-21 it carries only the perfumes that belong to no house, opening into `individual-fragrances/individual-fragrances.html`. |
| **the line** / **the pyramid** / **file** (fragrances) | The Fragrances view since 2026-09-25 (`fragrance-line.js`): a horizontal **line** through the window (two hairlines, where the houses' axis is one), a double **pyramid** of specks standing on it — a square base in the line, apices above and below — that turns a quarter turn for every file travelled, and the fragrances as **files** along it: the number large, the name under it, the house only on hover. The file in the middle opens its fragrance in the page. Twenty stand on it — seven real and thirteen **placeholders**, because the owner asked the test to assume twenty (`TEST_COUNT`). |
