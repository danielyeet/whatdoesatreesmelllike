...
- The 3D scene's hover effect was reworked: arms now show a triangle-
  wave "oscilloscope" ripple as the cursor gets close, instead of
  growing/moving. The bold-on-hover effect is toned down. Hovering a
  real node pauses the rotation and pulls nearby particles toward its
  arm. Particles now sit 1.5x further out, and the whole field is a
  little more low-key at rest.
- Clicking "Scent descriptions" now opens a pop-up preview instead of
  navigating away — a trial run before doing the same for the rest.
  See node-scene.js's `preview` field on a node to extend this to
  others once you're happy with how it looks.
